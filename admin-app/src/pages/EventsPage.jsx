...
const EventsPage = () => {
    ...
    const handleOpenCreate = () => {
        setIsEditing(false);
        setFormData({ title: '', description: '', date: '', time: '', venue: '', city: '', category: '', price: '', totalTickets: '', imageUrl: '' });
        setModalOpen(true);
    };
    ...
    const handleOpenEdit = (event) => {
        ...
        setFormData({
            ...
            time: event.time || formattedDate.slice(11, 16),
            venue: event.venue,
            city: event.city || '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing) {
                await api.put(`/admin/events/${currentEventId}`, payload);
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                            <div style={{ width: '100px', height: '6px', background: 'rgba(0,0,0,0.05)', borderRadius: '10px', overflow: 'hidden' }}>
                                                <div style={{ 
                                                    width: `${event.totalTickets ? (((event.totalTickets - event.availableTickets) / event.totalTickets) * 100) : 0}%`, 
                                                    height: '100%', 
                                                    background: 'var(--primary)' 
                                                }}></div>
                                            </div>
                                            <span style={{ fontSize: '12px', color: 'var(--text-muted)', minWidth: '60px' }}>
                                                {(event.totalTickets ?? 0) - (event.availableTickets ?? 0)} / {event.totalTickets ?? 0}
                                            </span>
                                        </div>
                                    </td>
                                    <td>
                    ...
                    <div className="glass-card premium-shadow" style={{ width: '100%', maxWidth: '600px', padding: '40px', position: 'relative' }}>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                ...
                                <div>
                                    <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '8px', color: 'var(--text-muted)' }}>TIME</label>
                                    <input type="time" value={formData.time} onChange={(e) => setFormData({...formData, time: e.target.value})} style={{ width: '100%' }} />
                                </div>
                                <div>
                                ...
                                <div>
                                    <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '8px', color: 'var(--text-muted)' }}>CITY</label>
                                    <input placeholder="Bengaluru" value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} style={{ width: '100%' }} />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <input type="number" placeholder="2000" required value={formData.totalTickets} onChange={(e) => setFormData({...formData, totalTickets: e.target.value})} style={{ width: '100%' }} />
                                </div>
                                <div style={{ gridColumn: 'span 2' }}>
                                    <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', marginBottom: '8px', color: 'var(--text-muted)' }}>POSTER IMAGE URL</label>
                                    <input placeholder="https://example.com/event.jpg" value={formData.imageUrl} onChange={(e) => setFormData({...formData, imageUrl: e.target.value})} style={{ width: '100%' }} />
                                </div>
                            </div>
