...
const statusStyle = (s) => {
  switch (s) {
    case 'upcoming':  return 'text-blue-500  border-blue-500/20  bg-blue-500/10';
    case 'ongoing':   return 'text-yellow-500 border-yellow-500/20 bg-yellow-500/10';
...
const EventManagement = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:5000/api/events/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error(`Server error ${res.status}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('[EventManagement] Saved event:', saved);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage your events, dates, pricing, and venues here.
          </p>
        </div>
        <button
